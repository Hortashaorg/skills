...
- `pnpm database zero` - Generate zero-schema.gen.ts from Drizzle schema (run after schema changes)
- `pnpm database migrate` - Apply database migrations
...
### Mutator/Query Folder Structure - Re-export in index.ts
When adding new mutators/queries, remember to export in `mutators/index.ts` or `queries/index.ts`:
```tsx
// mutators/index.ts
import * as newMutators from "./new-table.ts";
...
```
**Why**: Individual files in folders aren't automatically exported. Must manually re-export in index.ts.

### Zero Schema Generation - Always Run After Schema Changes
```bash
# After changing Drizzle schema in packages/database/db/
pnpm database zero  # Generates zero-schema.gen.ts
```
**Why**: `zero-schema.gen.ts` is auto-generated by `drizzle-zero` from your Drizzle schema. Never edit it manually - changes will be overwritten. After modifying database schema, always run `pnpm database zero` to regenerate types and zql queries.
