...
export const CommentThread = (props: CommentThreadProps) => {
	// Separate state for main comment input vs inline replies
	...
	const handleReply = (comment: Comment) => {
		...
		setReplyContent(`${quoted}\n\n`);
	};

	const handleNewCommentSubmit = () => {
		props.onSubmit?.(newComment(), null);
		setNewComment("");
	};

	const handleReplySubmit = () => {
		props.onSubmit?.(replyContent(), replyingTo());
		setReplyContent("");
		setReplyingTo(null);
	};
	...
	return (
		<Stack spacing="md">
			...
			{/* Comments - newest first (data should come pre-sorted from query) */}
